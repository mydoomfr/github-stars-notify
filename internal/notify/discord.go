...
func (d *DiscordNotifier) createMessage(owner, repo string, newStargazers []github.Stargazer) DiscordMessage {
	...
	embed := DiscordEmbed{
		Title:       fmt.Sprintf("New GitHub Stars"),
...
func (d *DiscordNotifier) sendMessage(ctx context.Context, message DiscordMessage) error {
	...
	if err != nil {
		return errors.NewNotificationError("discord", "failed to marshal message", err)
	}

	req, err := http.NewRequestWithContext(ctx, "POST", d.webhookURL, bytes.NewBuffer(jsonData))
	if err != nil {
		return errors.NewNotificationError("discord", "failed to create request", err)
	}
	...
	if err != nil {
		return errors.NewNotificationError("discord", "failed to send webhook", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return errors.NewNotificationError("discord",
