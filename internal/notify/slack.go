...
func (s *SlackNotifier) sendMessage(ctx context.Context, message SlackMessage) error {
	...
	if err != nil {
		return errors.NewNotificationError("slack", "failed to marshal message", err)
	}

	req, err := http.NewRequestWithContext(ctx, "POST", s.webhookURL, bytes.NewBuffer(jsonData))
	if err != nil {
		return errors.NewNotificationError("slack", "failed to create request", err)
	}
	...
	if err != nil {
		return errors.NewNotificationError("slack", "failed to send webhook", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return errors.NewNotificationError("slack",
