...
func NewForTest(cfg *config.Config) (*Service, error) {
	...
	if cfg.Notifications.Discord.Enabled {
		if notifier, err := notify.CreateBasicNotifier("discord", cfg.Notifications.Discord.WebhookURL); err == nil {
			notifiers = append(notifiers, notifier)
		}
	}
	if cfg.Notifications.Slack.Enabled {
		if notifier, err := notify.CreateBasicNotifier("slack", cfg.Notifications.Slack.WebhookURL, cfg.Notifications.Slack.Channel); err == nil {
			notifiers = append(notifiers, notifier)
		}
	}
...
func (s *Service) startMetricsServer() error {
	...
	s.logger.Info("metrics server started", "address", s.config.GetServerAddress())
	return nil
}
